...
function op_costSpikeTime( nodenr, timeIndices,command)
...
elseif strcmp(command,'backprop') == 1
    ...
    goalSpikes_cont = x*0;
    %resSpikes = x>0;
    resSpikes = [0 (diff(x>0)==1)];
    goalSpikes_cont = resSpikes*0;
    resSpksInds = find(resSpikes);
    for i = 1:length(goalSpikes)
        ...
        if (index > 0) && (index < length(x))
            ...
            if isempty(genSpikesCandidates)
            ...
            % Removal of existing spikes (multiple generated spikes close to real spike should be punished)
            for gi=1:length(genSpikesCandidates)
                gC = genSpikesCandidates(gi);
                
                delta_error = abs(gC-index);
                error_matrix(gC) = error_matrix(gC) + delta_error;
                
                % correct for shrinkage:
                m = sqrt((sum(x.^2)+2*x(gC)*delta_error+delta_error*delta_error)/sum(x.^2))-1;
